...
> **Bonus는 이제 한 화면에 여러 칸이 쌓이지 않습니다.** 예전에는 사이드바에서 "TODO 6"을
> 누르면 편집기 6개가 한꺼번에 세로로 쌓여서, 학생이 무엇부터 손대야 할지 알 수 없었습니다.
> 지금은 그 파트 하나하나가 **각자 사이드바 항목을 가진 독립 단계**(`6-1` … `9-4`)이고,
...
1. **움직임이 너무 느림/빠름** — TODO 6-2의 `PLAYER_MOVE_DELAY_MS`.
   `5000` 처럼 터무니없는 값을 넣으면 채점기가 경고하지만 **통과는 시킵니다**(개방형 원칙).
2. **라운드를 절대 못 깸** — TODO 9-3에서 "아이템 전부 수집" 조건을 넣었는데
   아이템이 0개인 라운드가 있는 경우. 역시 경고합니다.
3. **아이템 효과가 무한히 쌓임** — TODO 8-4에서 `item.active = False` 누락. 오답 처리됩니다.
4. **8-5·8-6에서 들여쓰기 오류** — 이 두 단계는 8-4가 연 `if` **안쪽**에 들어갑니다.
   8-4의 `for`/`if` 두 줄을 지우면 다음 단계가 문법 오류가 납니다.
